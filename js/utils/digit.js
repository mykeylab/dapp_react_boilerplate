...
// options: {
//   compare, double, floor
// }
function format(value, formatString, options) {
  if (_.isString(options)) {
    options = { compare: options };
  }
  const { compare, double, floor } = options || {};
  // 验证格式化字符串
  const re = /^(\^)?[~+-]?0(,(0|0{3}))?(\.(0+|#+))?%?$/;
  ...
  if (precision !== formatString.length) {
    if (floor) {
      output = value.toFixed10Floor(precision);
    } else {
  ...
  if (cls.length) {
    output = (
      <span className={`${cls}`}>{output}</span>
    );
  }
